...
class TestEmbibe(utility):
    ...
    @pytest.mark.usefixtures("setup", "log_on_failure")
    @pytest.mark.home
    def test_rl_filter_options_for_questions_to_revise(self):
        log = self.getLogger()
        self.test_sign_in_password()
        UH = UserHome(self.driver)
        UH.rl_filter_options_for_questions()
        log.info("Test case : User practices using different filter options in the Revision List")

    @pytest.mark.usefixtures("setup", "log_on_failure")
    @pytest.mark.rl
    def test_rl_filter_options_for_topics_to_revise(self):
        log = self.getLogger()
        self.test_sign_in_password()
        UH = UserHome(self.driver)
